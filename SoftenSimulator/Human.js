class Human {
    ...
    draw() {
        ...
        drawHuman(this.x*z0 + z0/2, this.y*z0 + z0/2);
        //gå på arbejde
        while (this.time > 0) {
            switch (this.state) {
                case "goToWork": 
    ...
    goHome() {
        if (this.wayHome.length > 0) {
            this.x = this.wayHome[this.wayHome.length-1].x;
            this.y = this.wayHome[this.wayHome.length-1].y;
            this.wayHome.splice(this.wayHome.length-1, 1);
        }        
    }

    walkToWork() {
        if (this.job != "unemployed") {
            this.wayHome.push({x: this.x, y:this.y});
            if (this.x != 100 && this.job.graph[this.x+1][this.y] < this.job.graph[this.x][this.y] && this.job.graph[this.x+1][this.y] !== false) {
                ...
                this.y --;
            } else if (this.job.graph[this.x][this.y] == 0) {
                this.state = "work";
            }
        }
    }
    ...
    update(i) {
        this.x = this.home.doorX;
        this.y = this.home.doorY;
        this.age += 4;
        // formering
...
function drawHuman(x, y) {
    push();
        fill(50,150,50);
        triangle(x, y-15, x-20, y+18, x+20, y+18);
        fill(50,50,180);
        rect(x-12, y+18, 5, 10);
        rect(x+7, y+18, 5, 10);
        fill(0);
